...
public class RecordSlider extends View {
    ...
    public RecordSlider(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public RecordSlider(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init(context, attrs, defStyleAttr);
    }


    public RecordSlider(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.RecordSlider, defStyleAttr, 0);

        // read all available attributes
        float startAngle = a.getFloat(R.styleable.RecordSlider_start_angle, (float) Math.PI / 2);
        float angle = a.getFloat(R.styleable.RecordSlider_angle, (float) Math.PI / 2);
        int thumbSize = a.getDimensionPixelSize(R.styleable.RecordSlider_thumb_size, 50);
        int thumbColor = a.getColor(R.styleable.RecordSlider_thumb_color, Color.GRAY);
        int borderThickness = a.getDimensionPixelSize(R.styleable.RecordSlider_border_thickness, 20);
        int borderColor = a.getColor(R.styleable.RecordSlider_border_color, Color.RED);
        String borderGradientColors = a.getString(R.styleable.RecordSlider_border_gradient_colors);
        Drawable thumbImage = a.getDrawable(R.styleable.RecordSlider_thumb_image);
    ...
    /**
     * Saves a new slider moved listener. Set {@link RecordSlider.OnSliderMovedListener} to {@code null} to remove it.
     *
